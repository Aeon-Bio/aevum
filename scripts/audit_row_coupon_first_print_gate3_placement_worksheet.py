...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Gate 3 OT-2 placement worksheet CSV path to audit.",
    )
    parser.add_argument(
        "--require-gate3-pass",
        action="store_true",
        help="Exit nonzero unless every Gate 3 placement row is pass-ready.",
    )
    ...
    print(f"gate3_placement_worksheet: {audit.worksheet_path}")
    print(f"worksheet_valid: {'true' if audit.worksheet_valid else 'false'}")
    print(f"gate3_pass_ready: {'true' if audit.gate3_pass_ready else 'false'}")
    print(f"expected_rows: {audit.expected_row_count}")
    print(f"actual_rows: {audit.actual_row_count}")
    for result in sorted(FIRST_PRINT_GATE3_PLACEMENT_RESULT_VALUES):
    ...
    if not audit.worksheet_valid:
        raise SystemExit(1)
    if args.require_gate3_pass and not audit.gate3_pass_ready:
        raise SystemExit(1)
