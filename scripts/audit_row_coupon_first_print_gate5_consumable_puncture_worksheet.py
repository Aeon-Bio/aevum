...
def _default_worksheet_path() -> Path:
    ...
    return ROOT / "data" / "measurements" / f"{today}_one_row_coupon_gate5_consumable_puncture.csv"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Gate 5 consumable/puncture worksheet CSV path to audit.",
    )
    parser.add_argument(
        "--require-gate5-pass",
        action="store_true",
        help="Exit nonzero unless every Gate 5 consumable/puncture row is pass-ready.",
    )
    ...
    print(f"gate5_consumable_puncture_worksheet: {audit.worksheet_path}")
    print(f"worksheet_valid: {'true' if audit.worksheet_valid else 'false'}")
    print(f"gate5_pass_ready: {'true' if audit.gate5_pass_ready else 'false'}")
    print(f"expected_rows: {audit.expected_row_count}")
    print(f"actual_rows: {audit.actual_row_count}")
    for result in sorted(FIRST_PRINT_GATE5_CONSUMABLE_PUNCTURE_RESULT_VALUES):
    ...
    if not audit.worksheet_valid:
        raise SystemExit(1)
    if args.require_gate5_pass and not audit.gate5_pass_ready:
        raise SystemExit(1)
