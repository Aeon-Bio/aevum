...
def _default_worksheet_path() -> Path:
    ...
    return ROOT / "data" / "measurements" / f"{today}_one_row_coupon_gate2_dry_assembly.csv"
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Gate 2 dry assembly worksheet CSV path to audit.",
    )
    parser.add_argument(
        "--require-gate2-pass",
        action="store_true",
        help="Exit nonzero unless every Gate 2 dry assembly row is pass-ready.",
    )
    ...
    print(f"gate2_dry_assembly_worksheet: {audit.worksheet_path}")
    print(f"worksheet_valid: {'true' if audit.worksheet_valid else 'false'}")
    print(f"gate2_pass_ready: {'true' if audit.gate2_pass_ready else 'false'}")
    print(f"expected_rows: {audit.expected_row_count}")
    print(f"actual_rows: {audit.actual_row_count}")
    for result in sorted(FIRST_PRINT_GATE2_DRY_ASSEMBLY_RESULT_VALUES):
    ...
    if not audit.worksheet_valid:
        raise SystemExit(1)
    if args.require_gate2_pass and not audit.gate2_pass_ready:
        raise SystemExit(1)
