...
def _row_coupon_export_models(params: dict[str, Any]) -> dict[str, cq.Workplane]:
    from aevum_cad.row_coupon import build_row_coupon_physical_artifacts

    return build_row_coupon_physical_artifacts(params)
...
def build_row_coupon_installed_parts(params: dict[str, Any]) -> dict[str, cq.Workplane]:
    ...
    physical = group_row_coupon_physical_artifacts_by_installed_part(
        params,
        assembly_position=True,
    )
    return {
        "deck_pods": physical["deck_pods"],
        "plate_support_frame": physical["plate_support_frame"],
        "ir_thermopiles": build_ir_thermopiles(params, assembly_position=True),
        "ir_thermopile_face_gaskets": physical["ir_thermopile_face_gaskets"],
        "lower_sensor_harness": build_lower_sensor_harness(params, assembly_position=True),
        "lower_harness_cover": physical["lower_harness_cover"],
        "lower_sensor_service_connector": build_lower_sensor_service_connector(
            params,
            assembly_position=True,
        ),
        "printed_lower_sensor_connector_shroud": physical[
        ...
        "lower_gasket": physical["lower_gasket"],
        "wet_chamber_frame": physical["wet_chamber_frame"],
        ...
        "upper_gasket": physical["upper_gasket"],
        "lid_manifold_shell": physical["lid_manifold_shell"],
        ...
        "lid_harness_cover": physical["lid_harness_cover"],
        "lid_sensor_service_connectors": build_lid_sensor_service_connectors(
            params,
            assembly_position=True,
        ),
        "printed_lid_sensor_connector_shrouds": physical[
        ...
        "lid_cover": physical["lid_cover"],
        "cots_gas_service_tubes": build_cots_gas_service_tubes(
            params,
            assembly_position=True,
        ),
        "gas_pcb_interface_gaskets": physical["gas_pcb_interface_gaskets"],
        "printed_gas_pcb_keeper_doors": physical["printed_gas_pcb_keeper_doors"],
        "gas_sensor_pcbs": build_gas_sensor_pcbs(params, assembly_position=True),
        "printed_sample_relief_cap": physical["printed_sample_relief_cap"],
        "printed_wedge_locks": physical["printed_wedge_locks"],
    }


def build_row_coupon_realized_installed_parts(
    params: dict[str, Any],
) -> dict[str, cq.Workplane]:
    """Build the full installed assembly from the exact final rigid release bodies.

    The 12 printed installed families come from the 38 final release bodies in
    installed coordinates.  The other 16 logical families retain their normal
    COTS, electronics, cable, tube, or consumable builders.
    """
    ...
    canonical = build_row_coupon_installed_parts(params)
    realized_printed = group_row_coupon_final_print_pieces_by_installed_part(
        params,
        assembly_position=True,
    )
    ...
    if set(realized_printed) != expected_printed:
        raise ValueError(
            "realized printed installed-family mismatch: "
            f"missing={sorted(expected_printed - set(realized_printed))}, "
            f"extra={sorted(set(realized_printed) - expected_printed)}"
        )
    if set(canonical) != set(installed_manifest):
        raise ValueError("canonical installed builders do not match the installed manifest")
    return {
        name: realized_printed.get(name, model)
        for name, model in canonical.items()
    }


def measure_row_coupon_realized_installed_pair_intersections(
    params: dict[str, Any],
) -> dict[str, float]:
    """Measure all 378 unordered installed-family pairs using final rigid bodies."""

    installed = build_row_coupon_realized_installed_parts(params)
    volumes: dict[str, float] = {}
    for left, right in combinations(sorted(installed), 2):
        key = "::".join((left, right))
        left_bbox = installed[left].val().BoundingBox()
        right_bbox = installed[right].val().BoundingBox()
        separated = (
            min(left_bbox.xmax, right_bbox.xmax) <= max(left_bbox.xmin, right_bbox.xmin)
            or min(left_bbox.ymax, right_bbox.ymax) <= max(left_bbox.ymin, right_bbox.ymin)
            or min(left_bbox.zmax, right_bbox.zmax) <= max(left_bbox.zmin, right_bbox.zmin)
        )
        volumes[key] = (
            0.0
            if separated
                ...
                float(shape.Volume())
                for shape in installed[left].intersect(installed[right]).vals()
            )
        )
    return volumes
